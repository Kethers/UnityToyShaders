    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i, bool IsFacing : SV_IsFrontFace) : SV_Target
            {
                ...
                float NoL = dot(N, L);  // Lambert
                float NoH = dot(N, H);  // Blinn-Phong
                float NoV = dot(N, V);  // Fresnel

                float3 normalVS = normalize(mul((float3x3)unity_MatrixV, N));
                ...
                baseColor = lerp(lerp(baseColor, baseColor * sphereTex.rgb, _SphereTexFac),
                                 lerp(baseColor, baseColor + sphereTex.rgb, _SphereTexFac), _SphereMulAdd);
                ...
                diffuse = lerp(darkShadowColor, diffuse, light.shadowAttenuation);
                ...
                float2 screenOffset = float2(lerp(-1, 1, step(0, normalVS.x)) * rimOffset / _ScreenParams.x / max(1, pow(_ScreenParams.x,2)), 0);
